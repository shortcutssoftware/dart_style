...
void main() {
    ...
    group('fileExists()', () {
        test('returns whether a file exists at that path', () async {
            ...
            var fs = IOFileSystem();
            expect(await fs.fileExists(await fs.makePath(p.join(d.sandbox, 'dir', 'exists.txt'))), isTrue);
            expect(await fs.fileExists(await fs.makePath(p.join(d.sandbox, 'dir', 'nope.txt'))), isFalse);
        });
        ...
        test('returns false if the entry at that path is not a file', () async {
            ...
            var fs = IOFileSystem();
            expect(await fs.fileExists(await fs.makePath(p.join(d.sandbox, 'dir', 'sub'))), isFalse);
        });
    });
    ...
    group('join()', () {
        test('joins paths', () async {
            var fs = IOFileSystem();
            expect((await fs.join(await fs.makePath('dir'), 'file.txt')).ioPath, p.join('dir', 'file.txt'));
        });
    ...
    group('parentDirectory()', () {
        ...
        test('returns the containing directory', () async {
            expect(await parent(p.join('dir', 'sub', 'file.txt')), p.absolute(p.join('dir', 'sub')));

            expect(await parent(p.join('dir', 'sub')), p.absolute(p.join('dir')));
        });

        test('normalizes path before getting parent', () async {
            expect(await parent(p.join('.', 'dir', 'sub')), p.normalize(p.absolute('dir')));
        });
    ...
    group('readFile()', () {
        test('reads a file', () async {
            ...
            expect(
                await fs.readFile(await fs.makePath(p.join(d.sandbox, 'dir', 'sub', 'another.txt'))),
                'more',
            );
        });
    ...
    group('readAnalysisOptions()', () {
        test('reports path correctly in exception', () async {
            await d.dir('dir', [d.file('options.yaml', 'include: package:foo/options.yaml')]).create();
            ...
            expect(
                ...
                throwsA(
                    isA<PackageResolutionException>().having(
                        ...
                        'message',
                        allOf(contains('package:foo/options.yaml'), contains('include at "${path.path}"')),
                    ),
                ),
            );
        });
