...
void main() {
    group('findAnalysisOptions()', () {
        ...
        test('finds a file in the given directory', () async {
            var testFS = TestFileSystem({'dir|analysis_options.yaml': analysisOptions(pageWidth: 100)});
    ...
    group('readAnalysisOptions()', () {
        ...
        test('merges included files', () async {
            var testFS = TestFileSystem({
                'dir|a.yaml': analysisOptions(
                    include: 'b.yaml',
                    other: {'a': 'from a', 'ab': 'from a', 'ac': 'from a', 'abc': 'from a', 'ad': 'from a'},
                ),
                'dir|b.yaml': analysisOptions(
                    include: ['c.yaml', 'd.yaml'],
                    other: {'ab': 'from b', 'abc': 'from b', 'b': 'from b', 'bc': 'from b', 'bd': 'from b'},
                ),
                'dir|c.yaml': analysisOptions(
                    other: {'ac': 'from c', 'abc': 'from c', 'bc': 'from c', 'c': 'from c', 'cd': 'from c'},
                ),
                'dir|d.yaml': analysisOptions(other: {'ad': 'from d', 'bd': 'from d', 'cd': 'from d'}),
            });
        ...
        test('resolves "package:" includes', () async {
            var testFS = TestFileSystem({
                'dir|a.yaml': analysisOptions(include: 'package:b/b_options.yaml', other: {'a': 'from a'}),
            ...
            expect(
                ...
                throwsA(
                    isA<PackageResolutionException>().having(
                        ...
                        'message',
                        allOf(contains('package:foo/options.yaml'), contains('include at "options.yaml"')),
                    ),
                ),
            );
        });
        ...
        test('ignores missing includes', () async {
            ...
            expect(failedReads, orderedEquals(['dir|unknown1.yaml', 'dir|unknown2.yaml']));
            expect(_pageWidth(options), equals(100));
        });
    });
}
