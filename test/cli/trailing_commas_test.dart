...
void main() {
    ...
    test('get mode from included options file', () async {
        await d.dir('foo', [
            ...
            d.dir('sub', [analysisOptionsFile(name: 'third.yaml', trailingCommas: TrailingCommas.preserve)]),
            d.file('main.dart', _unformatted),
        ]).create();
    ...
    group('stdin', () {
        test('use mode from surrounding package', () async {
            await d.dir('foo', [analysisOptionsFile(trailingCommas: TrailingCommas.preserve)]).create();
        ...
        test('ignore options file if mode is specified', () async {
            ...
            var process = await runFormatter(['--trailing-commas=preserve', '--stdin-name=foo/main.dart']);
