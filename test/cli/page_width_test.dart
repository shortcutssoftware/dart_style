...
void main() {
    ...
    test('ignore options file if page width specified on the CLI', () async {
        await d.dir('foo', [analysisOptionsFile(pageWidth: 20), d.file('main.dart', _unformatted)]).create();
    ...
    test('resolve "package:" includes', () async {
        await d.dir('dir', [
            ...
            d.dir('bar', [
                d.dir('lib', [analysisOptionsFile(include: 'package:baz/analysis_options.yaml')]),
            ]),
            d.dir('baz', [
    ...
    test('ignore missing includes', () async {
        await d.dir('dir', [
            analysisOptionsFile(include: ['unknown1.yaml', 'known.yaml', 'unknown2.yaml']),
        ...
        expect(await process.stderr.next, 'Warning: Couldn\'t read file "$unknown1Path".');

        var unknown2Path = p.join(d.sandbox, 'dir', 'unknown2.yaml');
        expect(await process.stderr.next, 'Warning: Couldn\'t read file "$unknown2Path".');
    ...
    test('only warn on a missing include once', () async {
        await d.dir('dir', [
            analysisOptionsFile(include: ['unknown.yaml', 'unknown.yaml', 'unknown.yaml']),
            d.file('main.dart', _unformatted),
        ]).create();
        ...
        expect(await process.stderr.next, 'Warning: Couldn\'t read file "$unknownPath".');
    ...
    group('stdin', () {
        ...
        test('ignore options file if page width is specified', () async {
            ...
            var process = await runFormatter(['--page-width=30', '--stdin-name=foo/main.dart']);
...
Future<void> _testWithOptions(Object? options, {required int expectedWidth}) async {
