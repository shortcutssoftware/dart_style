...
final class AstNodeVisitor extends ThrowingAstVisitor<void> with PieceFactory {
    ...
    void visitAssertInitializer(AssertInitializer node) {
        ...
        writeArguments(node.leftParenthesis, [node.condition, ?node.message], node.rightParenthesis);
    }
        ...
        pieces.withMetadata(node.metadata, () {
            ...
            pieces.add(
                ConstructorPiece(
                    ...
                    parameters,
                    body,
                    canSplitParameters: node.parameters.parameters.canSplit(node.parameters.rightParenthesis),
    ...
    void visitFormalParameterList(FormalParameterList node) {
        ...
        var firstOptional = node.parameters.indexWhere((p) => p.isNamed || p.isOptionalPositional);
        ...
        pieces.add(
            builder.build(
                forceSplit: style.preserveTrailingCommaBefore(node.rightDelimiter ?? node.rightParenthesis),
    ...
    void visitFunctionExpression(FunctionExpression node) {
        writeFunction(typeParameters: node.typeParameters, parameters: node.parameters, body: node.body);
    }
    ...
    void visitIsExpression(IsExpression node) {
        writeTypeTest(node.expression, node.isOperator, bang: node.notOperator, node.type);
    }
    ...
    void visitObjectPattern(ObjectPattern node) {
        ...
        pieces.add(builder.build(forceSplit: style.preserveTrailingCommaBefore(node.rightParenthesis)));
    }
    ...
    void visitPrefixExpression(PrefixExpression node) {
        ...
        var space = switch ((node.operator, node.operand)) {
            (Token(lexeme: '-' || '--'), PrefixExpression(operator: Token(lexeme: '-' || '--'))) => true,
            _ => false,
        };
    ...
    void visitRegularFormalParameter(RegularFormalParameter node) {
        if (node.functionTypedSuffix case var functionTypedSuffix?) {
            ...
            writeFormalParameter(node, node.type, node.name, mutableKeyword: node.constFinalOrVarKeyword);
        }
    }
    ...
    void visitStringInterpolation(StringInterpolation node) {
        for (var element in node.elements) {
            pieces.visit(
                element,
                context: node.isMultiline ? NodeContext.multilineStringInterpolation : NodeContext.none,
            );
        }
    }
    ...
    void visitSwitchExpressionCase(SwitchExpressionCase node) {
        var patternPiece = nodePiece(node.guardedPattern.pattern, context: NodeContext.switchExpressionCase);
                ...
                switch (member) {
                    ...
                    case SwitchPatternCase():
                        ...
                        if (member.guardedPattern.whenClause case var whenClause?) {
                            pieces.add(
                                InfixPiece([patternPiece, nodePiece(whenClause)], is3Dot7: style.is3Dot7),
                            );
                        } else {
        ...
        pieces.withMetadata(
            ...
            () {
                ...
                for (var variable in node.variables) {
                    if ((variable.equals, variable.initializer) case (var equals?, var initializer?)) {
                        if (style.is3Dot7) {
                ...
                pieces.add(
                    VariablePiece(header, variables, hasType: node.type != null, is3Dot7: style.is3Dot7),
                );
            },
        );
    }
