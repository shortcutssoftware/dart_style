...
final class TypeBuilder {
    ...
    TypeBuilder(
        ...
        NativeClause? nativeClause,
           ...
           _namePart = namePart,
           _clauses = [
               if (extendsClause case var clause?) _Clause(clause.extendsKeyword, [clause.superclass]),
               if (mixinOnClause case var clause?) _Clause(clause.onKeyword, clause.superclassConstraints),
               if (withClause case var clause?) _Clause(clause.withKeyword, clause.mixinTypes),
               if (implementsClause case var clause?) _Clause(clause.implementsKeyword, clause.interfaces),
               if (extensionOnClause case var clause?) _Clause(clause.onKeyword, [clause.extendedType]),
    ...
    void _buildType(Piece Function() buildBody, TypeBodyType bodyType) {
        _visitor.pieces.withMetadata(_metadata, () {
            if (_namePart case PrimaryConstructorDeclaration constructor) {
                ...
                _visitor.pieces.add(PrimaryTypePiece(header, parameters, clauses, bodyPiece, bodyType));
            } else {
    ...
    Piece _buildNormalBlockEnumBody(BlockEnumBody body) {
        ...
        return builder.build(
            forceSplit: _visitor.style.preserveTrailingCommaBefore(body.semicolon ?? body.rightBracket),
        );
    }
